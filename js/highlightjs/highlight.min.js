...
/* Basic helper: render KaTeX in .math-pane and highlight code in .code-pane.
   Usage in a post (see example below). */
document.addEventListener("DOMContentLoaded", function () {
  const interactiveBlocks = document.querySelectorAll(".interactive");
  interactiveBlocks.forEach(block => {
    const mathPane = block.querySelector(".math-pane");
    const codeNodes = block.querySelectorAll("pre code");

    // Render math if KaTeX auto-render function is available
    if (mathPane && window.renderMathInElement) {
      try {
        renderMathInElement(mathPane, {
          delimiters: [
    ...
    // Highlight code blocks with highlight.js if available
    if (window.hljs) {
      codeNodes.forEach(cb => {
        try {
